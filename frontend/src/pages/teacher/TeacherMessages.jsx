...
const TeacherMessages = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    if (chatLocked) {
      alert('Chat is locked. Payment not completed.');
      return;
    }
  ...
  const handleOpenChat = (enrollmentId) => {
    navigate(`/teacher/messages/${enrollmentId}`, { state: { from: '/teacher/messages' } });
  };

  const handleBackToList = () => {
    navigate('/teacher/messages', { state: { from: '/teacher/messages' } });
  };
            ...
            {enrollments.length === 0 ? (
              ...
              <div style={styles.chatList}>
                {enrollments.map((chat) => {
                  const otherUser = chat.other_user_name || 'Student';
  ...
  const otherUser = enrollment?.student_name || 'Student';
              ...
              Object.entries(groupedMessages).map(([date, msgs]) => (
                <React.Fragment key={date}>
                  ...
                  {msgs.map((msg) => {
                    ...
                    const senderName = isSender ? 'You' : (enrollment?.student_name || 'Student');
